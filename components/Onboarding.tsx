...
export const Onboarding: React.FC<Props> = ({ user, onComplete, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-lg bg-white rounded-[2rem] shadow-xl border border-slate-100 relative z-10 flex flex-col overflow-hidden">
          ...
          <div className="p-8 pb-6 text-center border-b border-slate-50 relative">
              ...
              <p className="text-slate-500 text-sm mt-2 font-medium">Step {step} of 2</p>
                          ...
                          {['CBSE', 'BSEB'].map(b => (
                              ...
                              >
                                  <div className="flex items-center gap-4">
                                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${board === b ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-blue-100 group-hover:text-blue-600'}`}>
                                          {b.charAt(0)}
                                      </div>
                                      <div className="text-left">
                                          <div className={`font-bold text-lg ${board === b ? 'text-blue-900' : 'text-slate-700'}`}>{b}</div>
                                          <div className="text-xs text-slate-400 font-medium">{b === 'CBSE' ? 'Central Board (English)' : 'Bihar Board (Hindi)'}</div>
                                      </div>
                                  </div>
          ...
          <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
              {step > 1 && (
                  <button
                      ...
                      className="px-6 py-4 rounded-xl font-bold text-slate-500 bg-white border border-slate-200 hover:bg-slate-100 transition-colors"
                  >
                      Back
                  </button>
              )}
      ...
      <div className="mt-8 text-center text-slate-400 text-xs font-bold uppercase tracking-widest relative z-10">
          Powered by NST AI
      </div>
    </div>
  );
