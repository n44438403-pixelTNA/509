...
const regex = /const renderDetailedSolutions = \(\) => \([\s\S]*?<\/\div>\n\s*\);\n\s*\};\n\s*const renderTopicBreakdown/;

// We need to find the correct ending bracket. It's better to just do a precise replace.
const oldStart = `  const renderDetailedSolutions = () => (
      <div className="space-y-6 mt-6">
          <h3 className="font-black text-slate-800 text-lg border-b pb-2">Detailed Solutions</h3>`;

const newStart = `  const renderDetailedSolutions = () => (
      <div className="mt-8">
