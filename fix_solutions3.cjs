...
const regex = /              return \(\s*<div key=\{idx\} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm break-inside-avoid relative group">[\s\S]*?<\/div>\s*\);\s*\}\)\}\s*<\/div>\s*\);\s*\};/;

const newItem = `              const isSkipped = userSelected === -1;
              return (
                      <div key={idx} className={\`mb-6 bg-white rounded-2xl border-2 p-5 shadow-sm break-inside-avoid relative group transition-all \${isCorrect ? 'border-green-100 hover:border-green-200' : isSkipped ? 'border-slate-200 hover:border-slate-300' : 'border-red-100 hover:border-red-200'}\`}>
